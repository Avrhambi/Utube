  ...
  useEffect(() => {
    const fetchVideoAndComments = async () => {
      try {
        ...
        try {
          const fallback = await fetch(`${process.env.PUBLIC_URL}/videos.json`);
          const videos = await fallback.json();
          const video = videos.find(v => v._id === pid);
          if (video) {
            setCurrentVideo({ ...video, likedBy: [], commentsList: [] });
            setVideoLikes(video.likes);
            setIsLiked(false);
          }
          setComments([]);
        } catch (e) {
          console.error('Failed to fetch the video:', e);
          setComments([]);
        }
      }
    };
    ...
    const fetchRecommendedVideos = async () => {
      try {
        ...
        try {
          const fallback = await fetch(`${process.env.PUBLIC_URL}/videos.json`);
          const videos = await fallback.json();
          setRecommendedVideos(videos.filter(v => v._id !== pid));
        } catch (e) {
          console.error('Failed to fetch the recommended videos:', e);
        }
      }
    };
